...
parser.add_argument('--lr_D', type=float, default=4e-4, help='learning rate for Adam optimizer (Discriminator)')
parser.add_argument('--lr_G', type=float, default=1e-4, help='learning rate for Adam optimizer (Generator)')
parser.add_argument('--gp_coeff', type=float, default=20, help='gradient penalty coefficient')
parser.add_argument('--cl_coeff', type=float, default=20, help='consistency loss coefficient')
...
parser.add_argument('--epoch_num', type=int, default=120, help='number of epochs to train')
...
def intermediate_plot(fake, fixed_labels, imgname, material_num=1):
...
def main():
    ...
    for epoch in range(1, epoch_num + 1):
        ...
        # if epoch % 25 == 0:
        #     fid_value = fid_main(netG, material_label)
        #     print(f"[Epoch {epoch}/{epoch_num}] FID: {fid_value:.4f}")
...
def visualize_results(seed_real=42, seed_fake=37, noise_in = None):
    ...
    col_num = int(np.clip(batch_size // 2, 1, 5))

    trainloader = torch.utils.data.DataLoader(trainset, batch_size=col_num*2, shuffle=False, drop_last=True)
    ...
    for i in range(4 * col_num):
        if i < 2 * col_num:
            ...
            axs[i].plot(fake_plt[i - 2 * col_num].flatten(), color='red')
            axs[i].set_xticks([])
            axs[i].set_yticks([])
            axs[i].set_ylim(0, 1)
